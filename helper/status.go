...
const (
	...
	CANCELLED           Code = 211
	UNKNOWN             Code = 212
	INVALID_ARGUMENT    Code = 213
	DEADLINE_EXCEEDED   Code = 214
	NOT_FOUND           Code = 215
	ALREADY_EXISTS      Code = 216
	PERMISSION_DENIED   Code = 217
	RESOURCE_EXHAUSTED  Code = 218
	FAILED_PRECONDITION Code = 219
	ABORTED             Code = 220
	OUT_OF_RANGE        Code = 221
	UNIMPLEMENTED       Code = 222
	INTERNAL            Code = 223
	UNAVAILABLE         Code = 224
	DATA_LOSS           Code = 225
	UNAUTHENTICATED     Code = 226
)

const (
	TRANSIENT_FAILURE string = "TRANSIENT_FAILURE"
...
/*
	case connectivity.Connecting:
				if delay, retry := c.retryer.Retry(fmt.Errorf(c.conn.GetState().String())); retry {
					if err := gax.Sleep(ctx, delay); err != nil {
						return err
					}
					c.logf.Info("Connecting")
					continue
				}
			...
			case connectivity.Shutdown:
				c.logf.Info("Shutdown")
				return nil

			case connectivity.TransientFailure:
				c.logf.Info("Transient Failure")
				return nil
